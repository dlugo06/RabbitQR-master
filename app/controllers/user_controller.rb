...
#  @user = User.where({:username => params[:user][:username], :password => params[:user][:password]})
  ...
  #        format.html { render :action => 'show' }
      format.html { redirect_to user_path(@user.id), notice: 'You have successfully logged in' }

      return
      # format.json { render :json => @user }
    else
      format.html { redirect_to login_form_path, notice: 'No user found, try again' }
      # render :json => @user
    end
  end
end
